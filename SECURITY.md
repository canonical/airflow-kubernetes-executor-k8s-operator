...
Vulnerabilities are tracked via [GitHub Security Advisories](https://github.com/canonical/airflow-kubernetes-executor-k8s-operator/security/advisories) and coordinated internally with Canonical's Product Security Incident Response Team (PSIRT).

Note that this charm manages Kubernetes resources for Airflow worker Pods but does not bundle a workload image itself — the OCI image used for worker Pods is operator-supplied via the `base_image` configuration option. Security of that image is the responsibility of the operator deploying the charm. Vulnerabilities in the upstream [Apache Airflow](https://github.com/apache/airflow) project should be reported to the [Apache Security Team](https://www.apache.org/security/) directly; Canonical will track and apply upstream fixes to supported tracks.
